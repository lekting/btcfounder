...
func FromMnemonic(mnemonic string) (*Generator, error) {
	var err error

	g := &Generator{}

	g.keyManager, err = NewKeyManager(128, "", mnemonic)
	if err != nil {
		return nil, err
	}
	return g, nil
...
// Returns a legacy address (P2PKH)
func (g *Generator) GetMainBTC() (string, string) {
	key, err := g.keyManager.GetKey(PurposeBIP44, CoinTypeBTC, 0, 0, uint32(0))
	if err != nil {
		log.Fatal(err)
	}
...
// Returns a p2sh-segwit address (P2WPKH-P2SH)
func (g *Generator) GetMainP2WPKHP2SH() (string, string) {
	key, err := g.keyManager.GetKey(PurposeBIP49, CoinTypeBTC, 0, 0, uint32(0))
	if err != nil {
		log.Fatal(err)
	}
	wif, _, _, segwitNested, err := key.Encode(true)
	if err != nil {
		log.Fatal(err)
	}

	return segwitNested, wif
}

// Returns a native segwit address (P2WPKH)
func (g *Generator) GetMainP2WPKH() (string, string) {
	key, err := g.keyManager.GetKey(PurposeBIP84, CoinTypeBTC, 0, 0, uint32(0))
	if err != nil {
		log.Fatal(err)
	}
	wif, _, segwitNative, _, err := key.Encode(true)
	if err != nil {
		log.Fatal(err)
	}

	return segwitNative, wif
}

func (g *Generator) Generate() (address, P2WPKH, P2WPKHP2SH string) {
	address, _ = g.GetMainBTC()
	P2WPKH, _ = g.GetMainP2WPKH()
	P2WPKHP2SH, _ = g.GetMainP2WPKHP2SH()
